...
def get_(domain = 'messages', package = 'base', languages = redREnviron.settings['language'], fallback = False):
    try:
        t = gettext.translation(domain, localedir = os.path.join(redREnviron.directoryNames['libraryDir'], package, 'languages'), languages = redREnviron.settings['language'], fallback = fallback)
